...
const UI_KEYS = Object.freeze([
  ...
  "bosporusShot", "lakeVictoriaShot", "assaultKyotoShot", "assaultRhodesShot"
]);
...
A globe-spanning sailing roguelike · 1522|Wishlist on Steam|Play browser demo|Launching November 18, 2026 on Steam for Windows, macOS, and Linux.
...
About the game|English screenshots|16 full-resolution gameplay screenshots with the English interface.|Download English screenshots
...
Developed & published by Iron Pagoda.|Privacy|Meet a panda in Sichuan|Sail the Great Barrier Reef|Sail the Spice Islands|Sail the Seto Inland Sea|Sail the Bosporus|Sail Lake Victoria|Assault Kyoto|Assault Rhodes
`),
  "zh-Hans": ui(`
游戏介绍|舰船|媒体资料|开发者问答（英文）|试玩版|语言
纵贯全球的航海 Roguelike · 1522|在 Steam 加入愿望单|在线试玩|将于2026年11月18日在 Steam 同步登陆 Windows、macOS 与 Linux。
船长日志|打开媒体资料|乘风启航。|在大海夺回一切之前赚取你的财富。
下载简体中文媒体包|资料一览|发行日期|平台|类型|文本语言
游戏介绍|简体中文截图|16张采用简体中文界面的全分辨率游戏截图。|下载简体中文截图
...
由 Iron Pagoda 开发并发行。|隐私|在四川遇见熊猫|航行大堡礁|航行香料群岛|航行濑户内海|航行博斯普鲁斯海峡|航行维多利亚湖|攻打京都|攻打罗德岛
`),
  ru: ui(`
Об игре|Корабли|Пресс-кит|Вопросы и ответы (англ.)|Демо|Язык
Морской roguelike на всей Земле · 1522|Добавить в желаемое Steam|Играть в браузере|Выходит 18 ноября 2026 года в Steam одновременно для Windows, macOS и Linux.
...
Об игре|Русские скриншоты|16 полноразмерных игровых скриншотов с русским интерфейсом.|Скачать русские скриншоты
...
Разработчик и издатель — Iron Pagoda.|Конфиденциальность|Встретить панду в Сычуани|Плавание вдоль Большого Барьерного рифа|Плавание среди Островов пряностей|Плавание по Внутреннему Японскому морю|Плавание по Босфору|Плавание по озеру Виктория|Штурм Киото|Штурм Родоса
`),
  es: ui(`
Acerca del juego|Navíos|Kit de prensa|Preguntas (inglés)|Demo|Idioma
Un roguelike naval por todo el globo · 1522|Añadir a deseados en Steam|Jugar la demo web|Lanzamiento simultáneo en Steam para Windows, macOS y Linux el 18 de noviembre de 2026.
...
Acerca del juego|Capturas en español|16 capturas de juego a resolución completa con la interfaz en español.|Descargar capturas en español
...
Desarrollado y publicado por Iron Pagoda.|Privacidad|Conoce a un panda en Sichuan|Navega por la Gran Barrera de Coral|Navega por las Islas de las Especias|Navega por el mar Interior de Seto|Navega por el Bósforo|Navega por el lago Victoria|Asalta Kioto|Asalta Rodas
`),
  "pt-BR": ui(`
Sobre o jogo|Navios|Kit de imprensa|Perguntas (inglês)|Demo|Idioma
Um roguelike de navegação pelo globo · 1522|Adicionar à lista da Steam|Jogar demo no navegador|Lançamento simultâneo na Steam para Windows, macOS e Linux em 18 de novembro de 2026.
...
Sobre o jogo|Capturas em português|16 capturas em resolução completa com a interface em português do Brasil.|Baixar capturas em português
...
Desenvolvido e publicado por Iron Pagoda.|Privacidade|Encontre um panda em Sichuan|Navegue pela Grande Barreira de Corais|Navegue pelas Ilhas das Especiarias|Navegue pelo Mar Interior de Seto|Navegue pelo Bósforo|Navegue pelo Lago Vitória|Assalte Quioto|Assalte Rodes
`),
  ja: ui(`
ゲーム紹介|船|プレスキット|開発者Q&A（英語）|体験版|言語
地球規模の航海ローグライク · 1522年|Steamでウィッシュリストに追加|ブラウザ体験版|2026年11月18日にSteamでWindows・macOS・Linux版を同時発売。
船長の航海日誌|プレスキットを開く|良い風をつかめ。|海に奪い返される前に、富を築け。
日本語プレスキットをダウンロード|基本情報|発売日|対応OS|ジャンル|対応言語
ゲーム紹介|日本語スクリーンショット|日本語インターフェイスのフル解像度ゲーム画面16点。|日本語スクリーンショットをダウンロード
...
Iron Pagoda 開発・販売。|プライバシー|四川でパンダに出会う|グレート・バリア・リーフを航海|香料諸島を航海|瀬戸内海を航海|ボスポラス海峡を航海|ヴィクトリア湖を航海|京都を攻略|ロドス島を攻略
`),
  de: ui(`
Über das Spiel|Schiffe|Pressekit|Fragen (Englisch)|Demo|Sprache
Ein weltumspannendes Seefahrt-Roguelike · 1522|Auf Steam wünschen|Browser-Demo spielen|Erscheint am 18. November 2026 gleichzeitig auf Steam für Windows, macOS und Linux.
...
Über das Spiel|Deutsche Screenshots|16 hochauflösende Spielszenen mit deutscher Oberfläche.|Deutsche Screenshots herunterladen
...
Entwickelt und veröffentlicht von Iron Pagoda.|Datenschutz|Einen Panda in Sichuan treffen|Am Great Barrier Reef segeln|Durch die Gewürzinseln segeln|Durch die Seto-Inlandsee segeln|Durch den Bosporus segeln|Auf dem Viktoriasee segeln|Kyoto erobern|Rhodos erobern
`),
  fr: ui(`
À propos|Navires|Dossier de presse|Questions (anglais)|Démo|Langue
Un roguelike de navigation à l'échelle du globe · 1522|Ajouter aux souhaits Steam|Jouer à la démo web|Sortie simultanée sur Steam pour Windows, macOS et Linux le 18 novembre 2026.
...
À propos du jeu|Captures françaises|16 captures de jeu en pleine résolution avec l'interface française.|Télécharger les captures françaises
...
Développé et publié par Iron Pagoda.|Confidentialité|Rencontrer un panda au Sichuan|Naviguez sur la Grande Barrière de corail|Naviguez dans les îles aux Épices|Naviguez sur la mer intérieure de Seto|Naviguez sur le Bosphore|Naviguez sur le lac Victoria|Assiégez Kyoto|Assiégez Rhodes
`),
  pl: ui(`
O grze|Statki|Materiały prasowe|Pytania (angielski)|Demo|Język
Żeglarski roguelike na całym globie · 1522|Dodaj do listy życzeń Steam|Zagraj w demo|Premiera jednocześnie na Steam dla Windows, macOS i Linux: 18 listopada 2026.
...
O grze|Polskie zrzuty ekranu|16 pełnowymiarowych zrzutów z polskim interfejsem.|Pobierz polskie zrzuty
...
Gra opracowana i wydana przez Iron Pagoda.|Prywatność|Spotkaj pandę w Syczuanie|Żegluj wzdłuż Wielkiej Rafy Koralowej|Żegluj po Wyspach Korzennych|Żegluj po Morzu Wewnętrznym Seto|Żegluj przez Bosfor|Żegluj po Jeziorze Wiktorii|Szturmuj Kioto|Szturmuj Rodos
`),
  "zh-Hant": ui(`
遊戲介紹|船艦|媒體資料|開發者問答（英文）|試玩版|語言
縱貫全球的航海 Roguelike · 1522|在 Steam 加入願望清單|線上試玩|將於2026年11月18日在 Steam 同步登上 Windows、macOS 與 Linux。
船長日誌|開啟媒體資料|乘風啟航。|在大海奪回一切之前賺取你的財富。
下載繁體中文媒體包|資料一覽|發售日期|平台|類型|文字語言
遊戲介紹|繁體中文截圖|16張採用繁體中文介面的完整解析度遊戲截圖。|下載繁體中文截圖
...
由 Iron Pagoda 開發並發行。|隱私|在四川遇見熊貓|航行大堡礁|航行香料群島|航行瀨戶內海|航行博斯普魯斯海峽|航行維多利亞湖|攻打京都|攻打羅德島
`),
  ko: ui(`
게임 소개|선박|프레스 키트|개발자 문답(영어)|데모|언어
전 세계를 누비는 항해 로그라이크 · 1522년|Steam 찜 목록에 추가|브라우저 데모 플레이|2026년 11월 18일 Steam에서 Windows, macOS, Linux용으로 동시 출시됩니다.
선장의 항해일지|프레스 키트 열기|좋은 바람을 잡으세요.|바다가 되찾기 전에 부를 이루세요.
한국어 프레스 키트 다운로드|기본 정보|출시|플랫폼|장르|지원 언어
게임 소개|한국어 스크린샷|한국어 인터페이스의 전체 해상도 게임 스크린샷 16장.|한국어 스크린샷 다운로드
...
Iron Pagoda 개발 및 배급.|개인정보 보호|쓰촨에서 판다 만나기|그레이트배리어리프 항해|향신료 제도 항해|세토 내해 항해|보스포루스 해협 항해|빅토리아호 항해|교토 공략|로도스 공략
`)
});
