...
The demo-trailer plan uses the in-game Mediterranean city theme before crossfading to the large-combat theme. Its longer edit replaces the fast ship montages with readable daylight voyages off Spain, in the Black Sea, on Lake Victoria, and along the Danube at Vienna. Capture scenarios declare whether modals are intentionally shown or suppressed; suppressed modals are closed after simulation and before every rendered frame. Build it with `npm run build:demo-trailer`; the output is `apps/pixel-globe/.captures/trailer/marque-and-reprisal-demo-trailer-v2.mp4`.
