...
The tracked edit plan, Pirata One end-card font, and builder live in `tools/trailer/`. Each plan names its primary and secondary music assets rather than relying on builder defaults. The main-trailer plan inserts two silent four-ship beam-reach montages on the score's 108 BPM and 85 BPM pulses. The builder validates that audible edit windows retain their required cannon, whale-kill, and trade cues, verifies the reviewed portrait and modal policies, and requires each combat shot to contain exactly one correctly sided player broadside with a visible hit. Captured sound events are rebuilt on one continuous trailer timeline so their natural tails can continue across picture cuts. The final storm footage continues in slow motion while the gameplay eases into Gaussian blur and the capsule-art title-and-ship lockup performs two rebounds before settling exactly. The builder writes `apps/pixel-globe/.captures/trailer/marque-and-reprisal-steam-trailer-v9.mp4`.

The demo-trailer plan uses the in-game Mediterranean city theme before crossfading to the large-combat theme. A galleass broadside appears in the first six seconds, without repeating its source later. The longer edit replaces the fast ship montages with readable daylight voyages off Spain, in the Black Sea, on Lake Victoria, and along the Danube at Vienna. Capture scenarios declare whether modals are intentionally shown or suppressed; suppressed modals are closed after simulation and before every rendered frame. Build it with `npm run build:demo-trailer`; the output is `apps/pixel-globe/.captures/trailer/marque-and-reprisal-demo-trailer-v3.mp4`.
