...
| Galleon | `galleon` | Sailing ship by cyc3w | three-masted galleon / armed merchant | Medium | Detailed mixed square-and-lateen rig replacing the generic Unity galleon. Its source textures are low-pass filtered for the 47px bake while retaining their authored material colors. |
...
| Fusta | `fusta` | Russian 22-bank Baltic galley | one-masted Mediterranean light galley | Medium | A compact derivative retaining only the central lateen rig, with three readable animated oars per side. Fast and agile, but cramped, lightly armed, and less seaworthy than a full galley. |
| Mediterranean Galley | `mediterranean-galley` | Russian 22-bank Baltic galley | sixteenth-century Mediterranean-style sailing galley | High | A lean two-sail derivative with four readable animated oars per side. |
...
| Spanish Nao | `spanish-nao` | Nao Victoria Galleon Ship | early-16th-century Spanish nao / small carrack | High | Spain-specific exploration hull based on Nao Victoria. Its unusually detailed dark texture is low-pass filtered and reduced to broad sail, trim, accent, and wood palettes during baking so the 47px sprite remains readable. |
| Portuguese Carrack | `portuguese-carrack` | Portuguese Carrack | early-16th-century Portuguese carrack | High | Portugal-specific armed ocean-going merchant retaining its cream-and-red sail treatment. Its detailed source materials are low-pass filtered during baking to suppress sub-pixel texture noise without recoloring the model. |
...
Cargo capacity is checked against both the average opaque area of the 32-heading runtime sprite and the baked waterline footprint. Merchant hulls may use that envelope efficiently; warships and oared vessels may carry substantially less because guns, fighting works, oars, and larger crews occupy space. The largest current merchant densities are the Urca and Javanese Jong, whose runtime silhouettes support their 520- and 600-unit holds without exceeding the fleet limits.
