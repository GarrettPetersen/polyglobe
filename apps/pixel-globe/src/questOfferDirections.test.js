...
for (const type of ["accept-quest", "open-passenger", "accept-passenger"]) {
  test(`${type} displays a heading without replacing sailing distance or eligibility`, () => {
    const quest = { id: "mission", destinationCityId: "east", distanceKm: 1300 };
    const option = { action: { type, quest }, detail: "1,300 km / 400 DB", disabled: true, disabledReason: "Hold full" };
    const result = questOfferDirections({ options: [option] }, { origin, citiesById, passengerQuest: quest });
    assert.equal(result.options[0].detail, "1,300 km to the east / 400 DB");
    assert.equal(result.options[0].disabledReason, "Hold full");
    assert.equal(option.detail, "1,300 km / 400 DB");
  });
}
test("capture destinations, date-line bearings and local tasks are handled explicitly", () => {
  const view = { options: [{ action: { type: "accept-quest", quest: { id: "capture", targetCityId: "north", distanceKm: 1267 } } }] };
  assert.equal(questOfferDirections(view, { origin, citiesById }).options[0].detail, "1,267 km to the north");
  ...
  assert.equal(questOfferDirections(view, { origin: {lat:0,lon:179}, citiesById:new Map([["north",{lat:0,lon:-179}]]) }).options[0].detail,"1,267 km to the east");
});
