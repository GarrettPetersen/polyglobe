...
export function shipPropulsionPerformance(stats, {
  ...
  if (stats.propulsion === SHIP_PROPULSION_OAR_SAIL) {
    const rowingMaxSpeed = stats.topSpeedRad * HYBRID_ROWING_SPEED_RATIO;
    return Object.freeze({
      accelerationFactor: Math.max(sailAcceleration, HYBRID_ROWING_ACCELERATION_RATIO),
      maxSpeedRad: Math.max(sailMaxSpeed, rowingMaxSpeed),
      stalled: false,
      rowing: rowingMaxSpeed >= sailMaxSpeed
    });
  }
