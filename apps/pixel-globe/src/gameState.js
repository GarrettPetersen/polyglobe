...
import {
  ...
  beginTradeEmbargoEnforcementCombat as markTradeEmbargoEnforcementCombat,
  ...
  recentTradeEmbargoEvents,
  resolveTradeEmbargoIncident,
  tradeEmbargoFine,
...
import {
  ...
  migrateCrewRecruitmentWounds,
  migrateCrewRosterWounds,
...
export const GAME_STATE_VERSION = 117;
...
export function createGameState({
  ...
  const state = {
    ...
    memory: {
      ...
      quests: {
        fishingTradeTutorial: createFishingTradeTutorialMemory(),
        active: null,
...
export function migrateGameState(state, shipStats, {
  ...
  const legacyIndividualCrewRoster = state.version >= 95
    ? migrateCrewRosterWounds(state.crewRoster)
    : null;
  const originMigratedCrewRoster = state.version >= 96
    ? legacyIndividualCrewRoster
    : state.version === 95
    ? migrateCrewRosterOriginTraits(legacyIndividualCrewRoster, (homePortCityId) => (
  ...
  const migrated = {
    ...
    crewPayroll: state.version >= 99
      ? state.crewPayroll
      : createCrewPayrollState(savedCrewExperienceMinute(state)),
    survival: {
      ...state.survival,
    ...
    memory: {
      ...
      quests: {
        ...
        fishingTradeTutorial: state.version >= 116
          ? state.memory.quests.fishingTradeTutorial
          : createFishingTradeTutorialMemory(),
...
export function advanceGamePolitics(state, currentMinute, { portCities = [], cities = portCities } = {}) {
  ...
  const embargoEvents = advanceTradeEmbargoPolitics(
    state.relations.tradeEmbargoes,
    state.relations.diplomacy,
    currentMinute,
    {
      authorityForFaction: (factionId) => sovereignAuthorityScore(
        state.relations.authority,
        factionId
      ),
      papalAuthority: papalAuthorityScore(state.relations.authority),
      inactiveFactionIds: state.memory.conquest.collapsedFactionIds
    }
  );
...
export function activeGameTradeEmbargoes(state) {
  assertGameState(state);
  return activeTradeEmbargoOrders(state.relations.tradeEmbargoes);
}

export function recentGameTradeEmbargoEvents(state, limit = 3) {
  assertGameState(state);
  return recentTradeEmbargoEvents(state.relations.tradeEmbargoes, limit);
}
...
function playerWorldDiplomacyInfluence(state) {
  ...
  const activeDiplomaticQuest = state.memory.quests?.envoyActive || null;
...
function shipConsumptionForValidatedState(state) {
  ...
  const carriedQuests = activeQuests(state.memory.quests);
  const passengers = travelerManifestCount(shipTravelerManifestForValidatedState(state));
  const livestock = carriedQuests.reduce((total, quest) => (
  ...
  const questFood = carriedQuests.reduce((total, quest) => (
    total + Math.max(0, Number(quest.consumption?.food || 0))
  ), 0);
  const questWater = carriedQuests.reduce((total, quest) => (
...
function shipTravelerManifestForValidatedState(state) {
  const groups = [];
  for (const quest of activeQuests(state.memory.quests)) {
    const group = activeQuestTravelerGroup(quest);
...
export function negotiateEnvoyQuest(state, city, context = {}) {
  assertGameState(state);
  const quests = questMemory(state);
  const active = quests.envoyActive;
...
export function grantEnvoySafePassage(state, factionId, simMinute) {
  ...
  assertSimulationMinute(simMinute);
  const id = assertFactionId(factionId);
  const active = questMemory(state).envoyActive;
...
function revokeSafePassageAfterAttack(state, factionId) {
  ...
  const activeQuest = questMemory(state).envoyActive;
...
function playerTradeEmbargoPurchaseOrders(state, city, goodId) {
  ...
  if (!sourceFactionId) return Object.freeze([]);
  return tradeEmbargoOrdersForPurchase(state.relations.tradeEmbargoes, {
    sourceFactionId,
    goodId,
...
export function createMarketUndoSnapshot(state, economy, city) {
  ...
  return Object.freeze({
    ...
    tradeEmbargoEnforcement: Object.freeze(structuredClone(
      state.memory.tradeEmbargoEnforcement
    )),
...
export function restoreMarketUndoSnapshot(state, economy, city, snapshot) {
  if (!snapshot || !Number.isFinite(snapshot.doubloons) || snapshot.doubloons < 0 ||
      ...
      !snapshot.decisions || !snapshot.factionReputation || !snapshot.tradeEmbargoEnforcement) {
  ...
  state.memory.tradeEmbargoEnforcement = structuredClone(snapshot.tradeEmbargoEnforcement);
...
export function tradeEmbargoInspectionStatus(state, incidentId) {
  assertGameState(state);
  const incident = tradeEmbargoIncidentById(state.memory.tradeEmbargoEnforcement, incidentId);
  const order = tradeEmbargoOrderById(state.relations.tradeEmbargoes, incident.orderId);
  const cargo = embargoCargoAvailable(incident, state.cargo);
  const fine = tradeEmbargoFine(incident);
  return Object.freeze({
    incident,
    order,
    cargo,
...
export function recordTradeEmbargoDetectionConsequences(state, incidentId) {
  const status = tradeEmbargoInspectionStatus(state, incidentId);
  const penalties = new Map([[
    status.incident.enforcingFactionId,
    TRADE_EMBARGO_REPUTATION_PENALTY
  ]]);
  if (status.order.authorityKind === TRADE_EMBARGO_AUTHORITY_PAPAL) {
    penalties.set(
      "papal-states",
      (penalties.get("papal-states") || 0) + PAPAL_EMBARGO_REPUTATION_PENALTY
    );
  }
  const reputationChanges = [];
  ...
  return Object.freeze({ ...status, reputationChanges: Object.freeze(reputationChanges) });
}

export function payTradeEmbargoFine(state, incidentId) {
  const status = tradeEmbargoInspectionStatus(state, incidentId);
  if (state.doubloons < status.fine) {
    throw new Error(`Not enough doubloons to pay a ${status.fine} db embargo fine`);
  }
  state.doubloons -= status.fine;
  resolveTradeEmbargoIncident(state.memory.tradeEmbargoEnforcement, incidentId);
  recordDecision(state, `trade.embargo.fine.${status.order.id}`, 1);
  return Object.freeze(status);
}

export function surrenderTradeEmbargoCargo(state, incidentId) {
  const status = tradeEmbargoInspectionStatus(state, incidentId);
  if (status.cargo.quantity <= 0) throw new Error("No embargoed cargo remains to surrender");
  resolveTradeEmbargoIncident(state.memory.tradeEmbargoEnforcement, incidentId);
  const removed = [];
  for (const [goodId, quantity] of Object.entries(status.cargo.manifest)) {
    trimCargoQuantity(state, goodId, Math.max(0, (state.cargo[goodId] || 0) - quantity));
    removed.push(Object.freeze({ goodId, quantity }));
  }
  recordDecision(
    state,
    `trade.embargo.cargo-surrendered.${status.order.id}`,
    removed.reduce((sum, row) => sum + row.quantity, 0)
  );
  return Object.freeze(removed);
}

export function beginTradeEmbargoEnforcementCombat(state, incidentId) {
  assertGameState(state);
  const incident = markTradeEmbargoEnforcementCombat(
    state.memory.tradeEmbargoEnforcement,
    incidentId
  );
  recordDecision(state, `trade.embargo.enforcement-fought.${incident.orderId}`, 1);
  return incident;
}

export function activeTradeEmbargoEnforcementFactionIds(state) {
  assertGameState(state);
  return Object.freeze(activeTradeEmbargoCombatFactionIds(state.memory.tradeEmbargoEnforcement));
}
...
export function visitPort(state, city, simMinute, { arrivedDrunk = false } = {}) {
  ...
  if (state.memory.campaignGoal) {
    recordCampaignGoalPortVisit(state.memory.campaignGoal, requireCityId(city, "Campaign port visit"));
  }
  if (arrivedDrunk) {
...
export function reconcileQuestPortTiles(state, portCities, {
  ...
  const reconcile = (quest) => {
    ...
    if (isWokouHuntQuest(quest)) {
      updates += reconcileQuestEndpoint(quest, "patrol", portCities, legacyPortTileIds);
    }
  };
  ...
  for (const settlement of colonization?.pastSettlements || []) {
    for (const endpoint of ["target", "origin", "approval"]) {
      updates += reconcileNamedPortFields(settlement, {
        ...
        countryField: `${endpoint}Country`
      }, portCities, legacyPortTileIds);
    }
    updates += reconcileNamedPortFields(settlement?.aftermath, {
      tileField: "reportTileId",
      nameField: "reportCity",
      countryField: "reportCountry"
    }, portCities, legacyPortTileIds);
  }
  updates += reconcileNamedPortFields(colonization?.aftermath, {
    tileField: "reportTileId",
    nameField: "reportCity",
    countryField: "reportCountry"
  }, portCities, legacyPortTileIds);
  ...
  const warLoan = quests.sovereignWarLoan;
  for (const record of [warLoan?.offer, warLoan?.contract]) {
    updates += reconcilePortIdentityFields(record, {
      tileField: "capitalTileId",
      portIdField: "capitalPortId"
    }, portCities, legacyPortTileIds);
  }
  updates += reconcilePortIdentityFields(warLoan?.contract?.security, {
    tileField: "tileId",
    portIdField: "portId",
    nameField: "portName"
  }, portCities, legacyPortTileIds);
...
export function completeQuest(state, city, context = {}) {
  ...
  const activeSlot = ACTIVE_QUEST_SLOTS.find(slot => quests[slot] &&
    (requestedQuestId ? quests[slot].id === requestedQuestId : quests[slot].destinationCityId === city.cityId)) || null;
...
function trimCargoQuantity(state, goodId, maximumQuantity) {
  ...
  consumeTrackedEmbargoCargo(state.memory.tradeEmbargoEnforcement, goodId, removed);
  return removed;
}
...
function reconciledPortReference(portCities, { cityId = null, tileId }, legacyPortTileIds) {
  ...
  const current = portCities.find((port) => port.tileId === resolvedTileId);
  if (migratedTileId !== undefined && !current) {
    throw new Error(
      `Saved port tile ${tileId} migrates to missing current port ${migratedTileId}`
    );
  }
  return current || null;
}

function reconcileBarePortField(record, field, portCities, legacyPortTileIds) {
  if (!record || !Number.isInteger(record[field])) return 0;
  const port = reconciledPortReference(portCities, { tileId: record[field] }, legacyPortTileIds);
  if (!port || port.tileId === record[field]) return 0;
  record[field] = port.tileId;
  return 1;
}
...
function reconcilePortIdentityFields(record, {
  tileField,
  portIdField,
  nameField = null
}, portCities, legacyPortTileIds) {
...
function assertLegacyPortTileIds(legacyPortTileIds) {
  if (legacyPortTileIds === null) return;
  if (!(legacyPortTileIds instanceof Map)) {
    throw new Error("Legacy quest port migration must be a Map");
  }
  for (const [savedTileId, currentTileId] of legacyPortTileIds) {
    if (!Number.isInteger(savedTileId) || !Number.isInteger(currentTileId)) {
      throw new Error(`Invalid legacy quest port migration: ${savedTileId}/${currentTileId}`);
    }
  }
}
...
function assertGameState(state) {
  ...
  validateTradeEmbargoMemory(state.relations?.tradeEmbargoes);
  ...
  validateExeterCanalState(state);
  ...
  validateFishingTradeTutorialMemory(state.memory.quests?.fishingTradeTutorial);
...
function assertDiplomaticQuestMemory(quests) {
  ...
  if (quests.envoyActive && !isEnvoyQuest(quests.envoyActive)) {
    throw new Error(`Non-envoy mission occupies envoy slot: ${quests.envoyActive.id}`);
  }
